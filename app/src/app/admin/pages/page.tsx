...
export default function AdminPagesPage() {
  ...
  useEffect(() => {
    if (!firestore) return;
    const pagesQuery = query(collection(firestore, 'pages'));

    const unsubscribe = onSnapshot(pagesQuery, (snapshot) => {
        const fetchedPages = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Page));
        const sorted = [...fetchedPages].sort((a, b) => (a.order ?? 999) - (b.order ?? 999));
        setPages(sorted);
