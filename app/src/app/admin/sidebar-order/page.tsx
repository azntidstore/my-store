...
export default function AdminSidebarOrderPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(sidebarOrderRef, (docSnap) => {
        if (docSnap.exists()) {
            const sidebarOrderData = docSnap.data() as {order: string[]};
        ...
        toast({ title: "خطأ", description: "فشل في تحميل ترتيب القائمة.", variant: "destructive" });
        setIsLoading(false);
    });

    return () => unsubscribe();

  }, [firestore, availableItems, toast]);
