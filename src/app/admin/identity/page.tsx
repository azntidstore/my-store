...
export default function AdminIdentityPage() {
  ...
  const settingsRef = useMemoFirebase(() => {
    ...
    return doc(firestore, 'settings', 'siteIdentity');
  }, [firestore]);

  const { data: settingsData, isLoading: loadingSettings, error } = useDoc<SiteSettings>(settingsRef);
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    useEffect(() => {
        // If there's no data in Firestore after loading, and we have a valid ref,
        // it means we need to initialize the document with default values.
        if (!loadingSettings && !settingsData && settingsRef) {
            onSubmit(form.getValues());
        }
    }, [loadingSettings, settingsData, settingsRef, form, onSubmit]);
